...
struct CoordVec coordvec_new_empty() {
	struct CoordVec new;
	new.len = 0;
	new.coords = (struct Coords *)calloc(0, 24 * sizeof(struct Coords));
	return new;
}
...
struct CoordVec coordvec_new_with_cap(int64_t cap) {
	struct CoordVec new;
	new.len = 0;
	new.coords = (struct Coords *)calloc(0, cap * sizeof(struct Coords));
}
...
void coordvec_push(struct CoordVec *vec, struct Coords new_coord) {
	...
	if(vec->len % 24 == 0) {
		vec->coords = realloc((void *)vec->coords, vec->len + 24);
	}
